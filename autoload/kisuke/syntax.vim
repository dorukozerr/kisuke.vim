function! kisuke#syntax#setup_menu()
  syntax clear

  syntax match KisukeMenuHeader /^•• ━━━━━━━━━━ ⟡ KISUKE ⟡ ━━━━━━━━━━ ••$/
  syntax match KisukeMenuItem /^✦ .*$/

  highlight def link KisukeMenuHeader Title
  highlight def link KisukeMenuItem Statement
endfunction

function! kisuke#syntax#setup()
...
function! kisuke#syntax#setup_incremental()
  let l:current_buf = bufnr('%')
...
function! s:setup_base_syntax()
  syntax match KisukePrompt /^Prompt >/
  syntax match KisukeResponse /^Kisuke >/
  syntax match KisukeSystem /^> .*$/

  highlight def link KisukePrompt Statement
  highlight def link KisukeResponse Identifier
  highlight def link KisukeSystem Special
  highlight def link KisukeSearch WarningMsg
  highlight def link KisukeFetch Function
  highlight def link KisukeInfo Comment
  highlight def link KisukeUsage Type
  highlight def link KisukeCodeDelimiter Delimiter
endfunction

function! s:process_code_blocks(buf_nr)
...
function! s:process_new_code_blocks(buf_nr)
...
function! s:find_and_highlight_complete_block(end_line, lang)
...
function! s:apply_syntax_to_block(lang, start_line, end_line)
  if empty(a:lang)
    return
  endif
