...
function! kisuke#buffer#create()
  execute 'vsplit ' . g:kisuke.state.buf_name

  let g:kisuke.state.buf_nr = bufnr('%')

  call kisuke#buffer#prepare_menu_buffer()

  call ch_sendraw(job_getchannel(g:kisuke.state.job), json_encode({ 'type': 'initialize', 'cwd': getcwd() }))
endfunction

function! kisuke#buffer#restore(payload = v:null)
  let l:checks = [
  ...
  if kisuke#utils#validate(l:checks)
    ...
    if l:wid ==# -1
      execute 'vsplit | buffer ' . g:kisuke.state.buf_nr
    else
...
function! kisuke#buffer#mark_focused_file()
...
function! kisuke#buffer#mark_highlighted_code() range
  let l:current_file = expand('%:p')
  let l:checks = [
        \ { 'condition': g:kisuke.state.job ==# v:null, 'message': 'Please run :KisukeOpen first' },
        \ { 'condition': g:kisuke.state.is_pending, 'message': 'Please wait for server to finish its job' },
...
function! kisuke#buffer#remove_last_marked_code_block()
  let l:checks = [
        \ { 'condition': g:kisuke.state.job ==# v:null, 'message': 'Please run :KisukeOpen first'},
        \ { 'condition': g:kisuke.state.is_pending, 'message': 'Please wait for server to finish its job'},
        \ { 'condition': bufwinid(g:kisuke.state.buf_nr) ==# -1, 'message': 'Please run :KisukeOpen first'},
...
function! kisuke#buffer#clear_marked_content()
...
function! kisuke#buffer#render_marked_content()
...
function! kisuke#buffer#on_submit(prompt)
  let l:checks = [
...
function! kisuke#buffer#prepare_chat_buffer()
  setlocal
        \ buftype=prompt
        \ noswapfile
        \ wrap
...
function! kisuke#buffer#prepare_menu_buffer()
  setlocal
        \ buftype=nofile
        \ bufhidden=hide
        \ noswapfile
