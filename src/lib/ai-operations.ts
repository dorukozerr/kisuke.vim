  ...
  try {
        ...
        messages: [
          {
            role: "user",
            content: context
              ? fileContextsProcessingInstructionsForStream(JSON.stringify(context), prompt)
              : prompt,
          },
        ],
        ...
        for await (const chunk of stream) {
          if (chunk?.type === "message_start" && chunk?.message?.model) {
            ...
            if (chunk.message.usage) {
              ...
              totalUsage.output_tokens += chunk.message.usage.output_tokens || 0;
          ...
          if (
            ...
            const needsSpacing = completeResponse.length > 0 && !completeResponse.endsWith("\n\n");
          ...
          if (chunk?.type === "message_delta" && chunk?.usage) {
            ...
            totalUsage.cache_creation_input_tokens += chunk.usage.cache_creation_input_tokens || 0;
            totalUsage.cache_read_input_tokens += chunk.usage.cache_read_input_tokens || 0;
        ...
        stdOutput({
          ...
          payload:
            error instanceof Error
              ? `Streaming error, ${JSON.stringify({ message: error.message, stack: error.stack })}`
              : "Streaming error, Unknown Error",
        });
      }
    ...
    if (config.provider === "google") {
      ...
      const contents = [
        {
          role: "model",
          text: BaseAIInstruction + sessionHistoryForStream(JSON.stringify(session)),
        },
        {
          role: "user",
          text: context
    ...
    if (config.provider === "openai") {
      ...
      const stream = await client.responses.create({
        model: config.model,
        instructions: BaseAIInstruction + sessionHistoryForStream(JSON.stringify(session)),
        input: [
          {
            role: "user",
            content: context
    ...
    if (config.provider === "grok") {
      ...
      const stream = await client.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: BaseAIInstruction + sessionHistoryForStream(JSON.stringify(session)),
          },
          {
            role: "user",
            content: context
