...
import type { Config, History, Session } from "#/types";

const configDir = join(homedir(), ".config", "kisuke");

if (!existsSync(configDir)) void mkdir(configDir, { recursive: true });
...
export const writeError = async (error: unknown, operation: string) => {
  ...
  try {
    ...
    if (Array.isArray(parsedContent)) {
      ...
      await fsWriteFile(errorFilePath, JSON.stringify([parsedContent], null, 2));
      currentErrors = [parsedContent];
    }
  } catch (error) {
...
const setupKisuke = async () => {
  ...
  return JSON.parse(await readFile(join(configDir, "history.json"), "utf-8")) as History;
};

export const getConfig = async () => {
  try {
    return JSON.parse(await readFile(join(configDir, "config.json"), "utf-8")) as Config;
  } catch (error) {
...
export const getSession = async (sessionId: string) => {
  try {
    return JSON.parse(await readFile(join(configDir, `${sessionId}.json`), "utf-8")) as Session;
  } catch (error) {
